...
/**
* Note:
* Dcode Denpa ( Script maker )
* Yuzzu Kamiyaka ( Mastah bot )
* Recode by 𝚈𝚊𝚖𝚊𝚒𝙱𝚘𝚝𝚣ッ
**/
...
waktuafk = 'Nothing'
alasanafk = 'Nothing'
NamaBot = recode.NamaBot
NomorOwner = recode.NomorOwner
NamaOwner = recode.NamaOwner
ovo = recode.Ovo
dana = recode.Dana
gopay = recode.Gopay
wm = recode.wm
...
module.exports = yamai = async (yamai, mek) => {
try {
        ...
        yamai.groupRemove(mek.key.remoteJid, [nums]).catch((e) => { reply(`*ERR:* ${e}`) })
        yamai.sendMessage(mek.key.remoteJid, tekuss, MessageType.text, {contextInfo:{mentionedJid:[nums + "@s.whatsapp.net"]}})
        }
		...
		yamai.on("CB:action,,battery", json => {
		...
		const dfrply = fs.readFileSync('./Images/Yamai.jpg')
		cmhit.push(command)
        mess = {
			wait: 'Wait a minute',
			success: 'Success',
		...
		const botNumber = yamai.user.jid
		const isGroup = from.endsWith('@g.us')
		const sender = mek.key.fromMe ? yamai.user.jid : isGroup ? mek.participant : mek.key.remoteJid
		const senderNumber = sender.split("@")[0] 
		const conts = mek.key.fromMe ? yamai.user.jid : yamai.contacts[mek.sender]
        const pushname = mek.key.fromMe ? yamai.user.name : !conts ? '-' : conts.notify || conts.vname || conts.name || '-'   
		const totalchat = await yamai.chats.all()
		const groupMetadata = isGroup ? await yamai.groupMetadata(from) : ''
		...
		const katalog = (teks) => {
             res = yamai.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© Dcode Denpa_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             yamai.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = yamai.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `${NamaBot}`, "footerText": "*_© Dcode Denpa_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
            yamai.relayWAMessage(grup)
        }
		idttt = []
	    players1 = []
	    players2 = []
		...
		try {
		pporang = await yamai.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
		pporang = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
		      }
		const ofrply = fs.readFileSync('./Images/Yamai1.jpg')
		const pporigi = fs.readFileSync('./Images/Yamai1.jpg')
...
const sendBug = async (target) => {
      await yamai.relayWAMessage(
        yamai.prepareMessageFromContent(
          target,
          yamai.prepareDisappearingMessageSettingContent(0),
          {}
        ),{ waitForAck: true }) 
    }
    //But verify
...
yamai.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
///Button Image
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await yamai.prepareMessage(from, kma, image)
...
yamai.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
///Button Video
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await yamai.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await yamai.prepareMessage(from, kma, location)
const buttonMessages = {
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const sendStickerUrl = async(to, url) => {
			console.log(color(time, 'magenta'), color(moment.tz('Asia/Jakarta').format('HH:mm:ss'), "gold"), color('Downloading sticker'))
				...
				download(url, namea, async function () {
					...
					yamai.sendMessage(to, media, sticker, {quoted: mek})
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        yamai.sendMessage(from, media, MessageType.sticker,{quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
	        ...
	        yamai.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
            }
            const hideTagKontak = async function(from, nomor, nama){
	        let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	        let anu = await yamai.groupMetadata(from)
	        let members = anu.participants
	        let ane = []
	        for (let i of members){
		    ane.push(i.jid)
	        }
	        yamai.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
            }
		const reply = (teks) => {
			yamai.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/qr/IZYUCENCHPWSC1`}}})
		}
		const simir = (teks) => {
			yamai.sendMessage(from, teks, text, { quoted:ftrol })
		}
		const math = (teks) => {
				return Math.floor(teks)
		}
		const sendMess = (hehe, teks) => {
			yamai.sendMessage(hehe, teks, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
		}
		//>>>>>>>>STICKER<<<<<<<\\
		const sticOwner = (hehe) => {
			ano = fs.readFileSync('./sticker/anime/owner.webp')
			yamai.sendMessage(hehe, ano, sticker, { quoted: mek})
		}
		...
		const mentions = (teks, memberr, id) => {
			(id == null || id == undefined || id == false) ? yamai.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : yamai.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
		}
		const sendFileFromUrl = async(link, type, options) => {
hasil = await getBuffer(link)
yamai.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
yamai.sendMessage(from, hasil, type, options).catch(e => {
yamai.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
  ...
  const sendMediaURL = async(to, url, text="", mids=[]) =>{
				...
				download(url, filename, async function () {
					...
					yamai.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
		   ...
		   const sendFakeThumb = async function(from, url, title, desc){
				...
				var qul = await yamai.generateLinkPreview(url)
				qul.title = title
				qul.description = desc
				qul.jpegThumbnail = dfrply
				yamai.sendMessage(from, qul, MessageType.extendedText, anoim)
			}
...
if (!mek.key.fromMe && !isOwner) return
try {
return yamai.sendMessage(from, JSON.stringify(eval(budy.slice(2)),null,'\t'),text, {quoted: mek})
} catch(err) {
e = String(err)
reply(e)
}
...
reply(String(e))
			...
			for (let anji of setik){
				if (budy === anji){
					...
					yamai.sendMessage(from, result, sticker, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					yamai.sendMessage(from, result, audio, { mimetype: 'audio/mp4', duration: 359996400, ptt: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					yamai.sendMessage(from, result, image, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
			}
        ...
        if (budy.includes("https://chat.whatsapp.com/")) {
        	if (!mek.key.fromMe){
				...
				yamai.updatePresence(from, Presence.composing)
				var kic = `${sender.split("@")[0]}@s.whatsapp.net`
				reply('Link terdeteksi, Auto kick!')
			    yamai.groupRemove(from, [kic]).catch((e) => { sticNotAdmin(from) })
			}
			}
			if (!m.key.fromMe && bad.includes(messagesD)) {
			ano = fs.readFileSync('./sticker/anime/toxic.webp')
			yamai.sendMessage(from, ano, sticker, { quoted: mek})
				}
...
numd = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=${numd.data.country_code}`)
                     sami = simi.success
                        yamai.sendMessage(from, `_${sami}_`, text, {thumbnail: ofrply, sendEphemeral: true, quoted:mek, contextInfo : {forwardingScore: 508, isForwarded: true}})
                      }
if (recode.autoread) {
yamai.chatRead(from)
}
if (!recode.autocomposing) {
yamai.updatePresence(from, Presence.composing)
}
if (!recode.autorecording) {
yamai.updatePresence(from, Presence.recording)
}
if (budy.startsWith('Yamai')){
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_𝚈𝚊𝚖𝚊𝚒𝙱𝚘𝚝𝚣ッ_*", [{buttonId: 'igme', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytme', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
reply('http://instagram.com')
}
if(isButton == 'ytm'){
reply('https://youtube.com/channel/UC44Dl_UKOchfXUAyR8QgbBQ')
}
...
mhan = await yamai.prepareMessage(from, ofrply, image, {thumbnail: ofrply})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${fill}`,
footerText: `${menu}`,
...
yamai.sendMessage(from, btnmenu, MessageType.buttonsMessage, {contextInfo :{text: 'hi',
...
"mediaType": "2",
"mediaUrl": "https://youtube.com/channel/UC44Dl_UKOchfXUAyR8QgbBQ",
"thumbnail": pporigi,
},mentionedJid:[stod]}, quoted : ftrol})
}
else if(menusimpel = true){
        stod = `${sender}`
       stst = await yamai.getStatus(`${sender.split('@')[0]}@c.us`)
...
├ _Auto Composing : ${recode.autocomposing}_
├ _Auto Recording : ${recode.autorecording}_
│
├───「 \`\`\`INFO USER\`\`\` 」
│
...
│
└───「 \`\`\`${NamaBot}\`\`\` 」`
sendButLocation(from, `${menu}`, `${wm}`, {jpegThumbnail:ofrply,name:""}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
break
case 'allmenu': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
l = 1
       stst = await yamai.getStatus(`${sender.split('@')[0]}@c.us`)
       nama = '🌹YAMAI KAZAMACHI 🌹'
       ownerJid = "6283844009539@s.whatsapp.net"
				stst = stst.status == 401 ? '' : stst.status
			num = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
menunya = `╭─❒ 「 Bot Info 」 ❒
├ _Creator : @${dtod.split('@')[0]}_
...
├ ${l++}. ${prefix}autorespon [ _on/off_ ]
├ ${l++}. ${prefix}antidelete [ _on/off_ ]
├ ${l++}. ${prefix}bc [ _teks/reply gif/image/video with caption_ ]
├ ${l++}. ${prefix}tobc [ _reply sticker/audio with caption_ ]
├ ${l++}. ${prefix}return [ _javascript_ ]
├ ${l++}. ${prefix}clearall
├ ${l++}. ${prefix}delchat
├ ${l++}. ${prefix}mute
├ ${l++}. ${prefix}unmute
├ ${l++}. ${prefix}public
├ ${l++}. ${prefix}self
├ ${l++}. ${prefix}spam [ _teks|jumlah_ ]
├ ${l++}. ${prefix}demoteall
├ ${l++}. ${prefix}promoteall
├ ${l++}. ${prefix}addcmd [ _teks reply stc_ ]
├ ${l++}. ${prefix}delcmd [ _reply stc_ ]
├ ${l++}. ${prefix}listcmd
├ ${l++}. ${prefix}spamsw [ _teks|jumlah_ ]
├ ${l++}. ${prefix}upswteks [ _teks_ ]
├ ${l++}. ${prefix}upswlokasi [ _teks_ ]
├ ${l++}. ${prefix}upswaudio [ _reply audio_ ]
├ ${l++}. ${prefix}upswvoice [ _reply audio_ ]
├ ${l++}. ${prefix}upswsticker [ _reply sticker_ ]
├ ${l++}. ${prefix}upswimage [ _reply image with caption_ ]
├ ${l++}. ${prefix}upswgif [ _reply gif with caption_ ]
├ ${l++}. ${prefix}upswvideo [ _reply video with caption_ ]
├ ${l++}. ${prefix}shutdown
├ ${l++}. ${prefix}offline [ _alasan_ ]
├ ${l++}. ${prefix}online
├ ${l++}. ${prefix}exif [ _nama|author_ ]
├ ${l++}. ${prefix}setprofile [ _reply image_ ]
├ ${l++}. ${prefix}setname [ _teks_ ]
├ ${l++}. ${prefix}setprefix [ _multi/nopref/prefix_ ]
├ ${l++}. ${prefix}setbio [ _teks_ ]
├ ${l++}. ${prefix}bug [ _jumlah_ ]
├ ${l++}. ${prefix}bugpc2 [ _jumlah_ ]
├ ${l++}. ${prefix}bugtroli2 [ _jumlah_ ]
├ ${l++}. ${prefix}bugpc
├ ${l++}. ${prefix}bugcombine
├ ${l++}. ${prefix}bugtroli
├ ${l++}. ${prefix}buglokasi
...
├ ${l++}. ${prefix}leave
├ ${l++}. ${prefix}restart
├ ${l++}. ${prefix}join [ _link group_ ]
├ ${l++}. ${prefix}addrespon [ _tanya|jawab_ ]
├ ${l++}. ${prefix}delrespon [ _nama_ ]
├ ${l++}. ${prefix}listrespon
├ ${l++}. ${prefix}readall
├ ${l++}. ${prefix}unreadall
├ ${l++}. ${prefix}archive
├ ${l++}. ${prefix}unarchiveall
├ ${l++}. ${prefix}pin
├ ${l++}. ${prefix}unpin
├ ${l++}. ${prefix}setmenu [ _ori/simpel_ ]
├ ${l++}. ${prefix}setallmenu [ _ori/simpel_ ]
├ ${l++}. ${prefix}leavetime [ _detik/menit/jam_ ]
├ ${l++}. ${prefix}bukatime [ _detik/menit/jam_ ]
├ ${l++}. ${prefix}tutuptime [ _detik/menit/jam_ ]
├ ${l++}. ${prefix}nano [ _nama file_ ]
│
├─❒ DOWNLOAD
├ ${l++}. ${prefix}infogempa
├ ${l++}. ${prefix}herolist
├ ${l++}. ${prefix}herodetail [ _hero_ ]
├ ${l++}. ${prefix}google [ _search_ ]
├ ${l++}. ${prefix}gimage [ _search_ ]
├ ${l++}. ${prefix}wiki [ _search_ ]
├ ${l++}. ${prefix}mediafire [ _link_ ]
├ ${l++}. ${prefix}ytsearch [ _judul_ ]
├ ${l++}. ${prefix}ytmp4 [ _link yt_ ]
├ ${l++}. ${prefix}ytmp3 [ _link yt_ ]
├ ${l++}. ${prefix}play [ _judul lagu_ ]
├ ${l++}. ${prefix}video [ _judul video_ ]
├ ${l++}. ${prefix}tinyurl [ _link_ ]
├ ${l++}. ${prefix}fetch [ _link_ ]
├ ${l++}. ${prefix}igdl [ _link_ ]
├ ${l++}. ${prefix}tiktokdl [ _link_ ]
├ ${l++}. ${prefix}pinterest [ _search_ ]
├ ${l++}. ${prefix}lirik [ _judul_ ]
├ ${l++}. ${prefix}tourl [ _reply image/video_ ]
├ ${l++}. ${prefix}resepmasakan [ _judul_ ]
├ ${l++}. ${prefix}artimimpi [ _teks_ ]
├ ${l++}. ${prefix}bilangangka [ _angka_ ]
├ ${l++}. ${prefix}kalkulator [ _angka_ ]
├ ${l++}. ${prefix}fancytext [ _teks_ ]
├ ${l++}. ${prefix}githubstalk [ _username_ ]
├ ${l++}. ${prefix}translate [ _teks kodebhs_ ]
├ ${l++}. ${prefix}ss [ _link_ ]
├ ${l++}. ${prefix}playstore [ _search_ ]
├ ${l++}. ${prefix}tiktokaudio [ _link_ ]
├ ${l++}. ${prefix}brainly [ _search_ ]
├ ${l++}. ${prefix}igstory [ _search_ ]
├ ${l++}. ${prefix}igstalk [ _link_ ]
├ ${l++}. ${prefix}twitter [ _link_ ]
├ ${l++}. ${prefix}twmp3 [ _link_ ]
├ ${l++}. ${prefix}linkwa [ _search_ ]
├ ${l++}. ${prefix}fb [ _link_ ]
├ ${l++}. ${prefix}chara [ _search_ ]
├ ${l++}. ${prefix}otaku [ _search_ ]
├ ${l++}. ${prefix}komiku [ _search_ ]
│
├─❒ MAKER
├ ${l++}. ${prefix}foliokanan [ _text_ ]
├ ${l++}. ${prefix}foliokiri [ _text_ ]
├ ${l++}. ${prefix}nuliskanan [ _text_ ]
├ ${l++}. ${prefix}nuliskiri [ _text_ ]
├ ${l++}. ${prefix}maker [ _text_ ]
├ ${l++}. ${prefix}maker2 [ _text_ ]
├ ${l++}. ${prefix}maker3 [ _text_ ]
├ ${l++}. ${prefix}maker4 [ _text_ ]
├ ${l++}. ${prefix}maker2d2 [ _text_ ]
├ ${l++}. ${prefix}maker2d3 [ _text_ ]
├ ${l++}. ${prefix}maker2d4 [ _text_ ]
├ ${l++}. ${prefix}maker3d [ _text_ ]
├ ${l++}. ${prefix}maker3d2 [ _text_ ]
├ ${l++}. ${prefix}maker3d3 [ _text_ ]
├ ${l++}. ${prefix}maker3d4 [ _text_ ]
├ ${l++}. ${prefix}transformer [ _text_ ]
├ ${l++}. ${prefix}googletxt [ _text_ ]
├ ${l++}. ${prefix}battlefield [ _text_ ]
├ ${l++}. ${prefix}coffeecup [ _text_ ]
├ ${l++}. ${prefix}coffeecup2 [ _text_ ]
├ ${l++}. ${prefix}neon [ _text_ ]
├ ${l++}. ${prefix}glow [ _text_ ]
├ ${l++}. ${prefix}summer [ _text_ ]
├ ${l++}. ${prefix}flower [ _text_ ]
├ ${l++}. ${prefix}burn [ _text_ ]
├ ${l++}. ${prefix}quote [ _text_ ]
├ ${l++}. ${prefix}wooden [ _text_ ]
├ ${l++}. ${prefix}golden [ _text_ ]
├ ${l++}. ${prefix}passed [ _reply gambar_ ]
├ ${l++}. ${prefix}gay [ _reply gambar_ ]
├ ${l++}. ${prefix}glass [ _reply gambar_ ]
├ ${l++}. ${prefix}sampah [ _reply gambar_ ]
├ ${l++}. ${prefix}trigger [ _reply gambar_ ]
├ ${l++}. ${prefix}greyscale [ _reply gambar_ ]
├ ${l++}. ${prefix}hijau [ _reply gambar_ ]
├ ${l++}. ${prefix}biru [ _reply gambar_ ]
├ ${l++}. ${prefix}comrade [ _reply gambar_ ]
├ ${l++}. ${prefix}jail [ _reply gambar_ ]
├ ${l++}. ${prefix}blurple2 [ _reply gambar_ ]
├ ${l++}. ${prefix}red [ _reply gambar_ ]
├ ${l++}. ${prefix}blurple [ _reply gambar_ ]
├ ${l++}. ${prefix}invert [ _reply gambar_ ]
├ ${l++}. ${prefix}invert_greyscale [ _reply gambar_ ]
├ ${l++}. ${prefix}sepia [ _reply gambar_ ]
├ ${l++}. ${prefix}pelangi [ _reply gambar_ ]
├ ${l++}. ${prefix}wasted [ _reply gambar_ ]
├ ${l++}. ${prefix}matrix [ _teks_ ]
├ ${l++}. ${prefix}googletxt [ _teks_ ]
├ ${l++}. ${prefix}spiderman [ _teks_ ]
├ ${l++}. ${prefix}express [ _teks_ ]
├ ${l++}. ${prefix}dance [ _teks_ ]
├ ${l++}. ${prefix}blackbird [ _teks_ ]
├ ${l++}. ${prefix}halloween [ _teks_ ]
├ ${l++}. ${prefix}vampire [ _teks_ ]
├ ${l++}. ${prefix}codetxt [ _teks_ ]
├ ${l++}. ${prefix}text3d [ _teks_ ]
├ ${l++}. ${prefix}warrior [ _teks_ ]
│
├──❒ GROUP
├ ${l++}. ${prefix}getpict [ _@tag_ ]
├ ${l++}. ${prefix}getname [ _reply target_ ]
├ ${l++}. ${prefix}getbio [ _reply target_ ]
├ ${l++}. ${prefix}promote [ _@tag_ ]
├ ${l++}. ${prefix}demote [ _@tag_ ]
├ ${l++}. ${prefix}antilink [ _on/off_ ]
├ ${l++}. ${prefix}antibug [ _on/off_ ]
├ ${l++}. ${prefix} welcome [ _on/off_ ]
├ ${l++}. ${prefix}creategrup [ _nama|@tag_ ]
├ ${l++}. ${prefix}tictactoe [ _@tag_ ]
├ ${l++}. ${prefix}delttt
├ ${l++}. ${prefix}getpp
├ ${l++}. ${prefix}kick [ _@tag_ ]
├ ${l++}. ${prefix}add [ _nomor_ ]
├ ${l++}. ${prefix}getdeskgc
├ ${l++}. ${prefix}sider [ _reply pesan bot_ ]
├ ${l++}. ${prefix}hacked [ _teks_ ]
├ ${l++}. ${prefix}setnamegc [ _teks_ ]
├ ${l++}. ${prefix}setdeskgc [ _teks_ ]
├ ${l++}. ${prefix}fitnah [ _@tag|teks1|teks2_ ]
├ ${l++}. ${prefix}kontak [ _@tag|nama_ ]
├ ${l++}. ${prefix}kontag [ _@tag|nama_ ]
├ ${l++}. ${prefix}opengc
├ ${l++}. ${prefix}closegc
├ ${l++}. ${prefix}resetlinkgc
├ ${l++}. ${prefix}linkgrup
├ ${l++}. ${prefix}hidetag [ _teks_ ]
├ ${l++}. ${prefix}sticktag [ _nama sticker_ ]
├ ${l++}. ${prefix}totag [ _reply media_ ]
│
├─❒ TOOLS
├ ${l++}. ${prefix}sticker
├ ${l++}. ${prefix}stickerwm [ _nama|author_ ]
├ ${l++}. ${prefix}takestick [ _nama|author_ ]
├ ${l++}. ${prefix}colong [ _reply sticker_ ]
├ ${l++}. ${prefix}dadu
├ ${l++}. ${prefix}semoji [ _emoji_ ]
├ ${l++}. ${prefix}attp [ _teks_ ]
├ ${l++}. ${prefix}toimg
├ ${l++}. ${prefix}tomp3 [ _reply video_ ]
├ ${l++}. ${prefix}tomp4 [ _reply sticker gif_ ]
├ ${l++}. ${prefix}robot [ _reply audio_ ]
├ ${l++}. ${prefix}balik [ _reply audio_ ]
├ ${l++}. ${prefix}bass [ _reply audio_ ]
├ ${l++}. ${prefix}gemuk [ _reply audio_ ]
├ ${l++}. ${prefix}detikvn [ _reply audio caption angka_ ]
├ ${l++}. ${prefix}detikvideo [ _reply video caption angka_ ]
│
├─❒ ASUPAN
├ ${l++}. ${prefix}asupan
├ ${l++}. ${prefix}asupan2
...
├ ${l++}. ${prefix}listvn
├ ${l++}. ${prefix}addsticker [ _nama_ ]
├ ${l++}. ${prefix}delsticker [ _nama_ ]
├ ${l++}. ${prefix}addvn [ _nama_ ]
├ ${l++}. ${prefix}delvn [ _nama_ ]
├ ${l++}. ${prefix}addimage [ _nama_ ]
├ ${l++}. ${prefix}delimage [ _nama_ ]
│
├─❒ JADI BOT
├ ${l++}. ${prefix}stopjadibot
├ ${l++}. ${prefix}jadibot
...
mhan = await yamai.prepareMessage(from, pporigi, image, {thumbnail: pporigi})
const btnBngsat = {
imageMessage: mhan.message.imageMessage,
contentText: `${menunya}`,
footerText: `${wm}`,
...
yamai.sendMessage(from, btnBngsat, MessageType.buttonsMessage, { quoted: ftrol, caption: menunya, contextInfo: { mentionedJid: [dtod,otod,stod]}})
                  }
break
case 'verify':
...
yamai.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:ftrol, contextInfo: { mentionedJid: [sender]}})
...
yamai.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:ftrol})
break
case 'ownermenu':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
menu = `❏ 「 \`\`\`MENU OWNER\`\`\` 」
...
yamai.sendMessage(from, anu, image, { quoted: mek, thumbnail: fs.readFileSync('./Images/Yamai.jpg')})
break
case 'nuliskanan':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!c) return reply('Textnya mana gan?')
...
yamai.sendMessage(from, asu, video, { quoted: mek})
break
case 'asupan2':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
...
yamai.sendMessage(from, ano, video, { quoted: mek})
break
case 'asupant3':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  sticWait(from)
...
ano = await getBuffer(anu.result.url)
yamai.sendMessage(from, ano, image, { quoted: mek, thumbnail: fs.readFileSync('./Images/Yamai.jpg')})
break
...
break
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, buffer1, image, {quoted: mek, thumbnail: fs.readFileSync('./Images/Yamai.jpg')})
					break
		case 'maker2d3': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (args.length < 1) return reply(`*Example :*\n${prefix + command} Yuzzu Kamiyaka`)
...
mhan = await yamai.prepareMessage(from, buff, image, {thumbnail: buff})
...
yamai.sendMessage(from, buttonMessagessss, MessageType.buttonsMessage, {
        thumbnail: fs.readFileSync('./Images/Yamai.jpg'),
        caption: 'Tes',
            quoted: mek})
break
//🐀💰 MALING
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					owgi = await  yamai.downloadAndSaveMediaMessage(ger)
					...
					yamai.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
					} else {
					reply('Gunakan foto!')
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, buffer, image, {quoted: mek})
                   }
              break       
		case 'gay':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
		            })              
					break
case 'igstalk':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
yamai.sendMessage(from, nih_buff, image, { quoted: mek, caption: tt });
}
break;      
break    
                case 'bisakah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
				case 'kapankah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					break
         			  case 'apakah':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					apakah = body.slice(1)
					...
					yamai.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					break
				case 'rate':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					rate = body.slice(1)
					...
					yamai.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					break
		    case 'tiktokaudio':
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	...
	yamai.sendMessage(from,nowmm ,MessageType.audio,{mimetype:'audio/mp4',quoted: mek})
		})
		})
		break 
	case 'igstory': 
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            for(let i of result.medias){
                if(i.url.includes('mp4')){
                    ...
                    yamai.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
                } else {
                    let link = await getBuffer(i.url)
                    yamai.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
                }
            }
            });
            break
...
${anime.desc}\n\n*Link Batch* : ${anime.batch}\n*Link Download SD* : ${anime.batchSD}\n*Link Download HD* : ${anime.batchHD}`
            ...
            yamai.sendMessage(from,ram,image,{quoted:mek,caption:rem})
            break
    case 'komiku':
    if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
Anu = res.SD
fto = fs.readFileSync('./Images/Yamai.jpg')
...
yamai.sendMessage(from, khs, audio, {mimetype:'audio/mp4', filename:'audio.mp3', quoted:mek, ptt:true})
})
break
    case 'chara':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            await yamai.sendMessage(from,li,image,{quoted: mek})
            break
//Navi
					          case 'unpin':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                yamai.modifyChat(from, ChatModification.unpin)
                reply('*succes unpin this chat*')
                console.log('unpin chat = ' + from)
                break
            case 'pin':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                yamai.modifyChat(from, ChatModification.pin)
             ...
             case 'archive':
                ...
                await sleep(3000)
                yamai.modifyChat(from, ChatModification.archive)
                break
            case 'unreadall':
                if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                var chats = await yamai.chats.all()
                chats.map( async ({ jid }) => {
                await yamai.chatRead(jid, 'unread')
                    })
		    var teks = `\`\`\`Successfully unread ${chats.length} chats !\`\`\``
		    await yamai.sendMessage(from, teks, text, {quoted: mek})
		...
		break
            case 'unarchiveall':
                ...
                anu = await yamai.chats.all()
                for (let _ of anu) {
                yamai.modifyChat(_.jid, ChatModification.unarchive)
                }
                break
case 'tutuptime':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
yamai.groupSettingChange (from, GroupSettingChange.messageSend, true);
}, timer)
break
case 'bukatime':
		if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
setTimeout( () => {
var nomor = mek.participant
yamai.groupSettingChange (from, GroupSettingChange.messageSend, false);
}, timer)
break
case 'leavetime':
if (!isGroup) return reply(mess.only.group)
...
await yamai.sendMessage(from, skuygelud, MessageType.buttonsMessage, {quoted: ftrol})
}
break
case 'setallmenu':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
await yamai.sendMessage(from, skuygf, MessageType.buttonsMessage, {quoted: ftrol})
}
break
//Sampai sini
case 'jadibot':
...
let scen = await yamai.sendMessage(from, fs.readFileSync(`./jadibot@${sender}.jpg`), MessageType.image, {quoted : mek,caption: 'Scan QR ini untuk jadi bot sementara!\n1. Klik titik tiga di pojok kanan atas\n2. Ketuk WhatsApp Web\n3. Scan QR ini \n\nQR Expired dalam 20 detik'})    
setTimeout(() => {
       yamai.deleteMessage(from, scen.key)
  }, 30000);
})  
...
const vname = yamai.contacts[i] != undefined ? yamai.contacts[i].vname || yamai.contacts[i].notify : undefined
ini_list.push({
"displayName": `Developer ${NamaBot}`,
"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Dcode Denpa;;;\nFN:${vname ? `${vname}` : `${yamai.user.name}`}\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
})
}
yamai.sendMessage(from, {
"displayName": `Developer ${NamaBot}`,
"contacts": ini_list 
...
└──────[ GITHUB ]──────❒
  ...
  │◦➛Script Recode2 : 
  │◦➛https://github.com/YamaiKazamachi/bitch-but
  └──────────────────❒`
 buttons = [
...
await yamai.sendMessage(from, btnskuy, MessageType.buttonsMessage, {quoted: ftrol})
		break
case 'donasi':
...
yamai.sendMessage(from, dona, image, { quoted: ftrol, thumbnail: dona, caption: doni })
break
       case 'debug':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			 res = await yamai.prepareMessageFromContent(from,{
"templateMessage": {
						"hydratedTemplate": {
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            owgi = await yamai.downloadMediaMessage(boij)
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    ...
    let qse = yamai.serializeM(await m.getQuotedObj())
...
case 'listgroup':
                ...
                case 'grouplist':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
  const txs = yamai.chats.all().filter(v => v.jid.endsWith('g.us')).map(v =>`- ${yamai.getName(v.jid)}\n${v.jid}\n[${v.read_only ? 'Left' : 'Joined'}]`).join`\n\n`
  reply(txs)
  break
  case 'caripesan':
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
cok = await yamai.searchMessages(`${ve}`, from, batas,1) 
if (cok.messages.lenght < 2) return reply('Pesan tidak ditemukan!') 
if (cok.messages.length < parseInt(batas)) reply(`Hanya ditemukan ${cok.messages.length - 1} Pesan`)
for (let i=1;i < cok.messages.length;i++) {
if (cok.messages[i].message) {
yamai.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
await yamai.sendMessage(from, btnharam, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
                case 'antidelete':
      if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
await yamai.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: ftrol})
                }
                break
                case 'setprefix':
      if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
yamai.sendMessage(from, `*🎳 Memulai Game Tictactoe 🎲*
...
break
		...
		try {
					ppimg = await yamai.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await yamai.getProfilePicture(anu)
...
await yamai.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
			publik = true
...
break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				anu = await fetchJson(`https://bx-hunter.herokuapp.com/api/resepmakanan?query=${teks}&apikey=${HunterApi}`, {method: 'get'})
					...
					yamai.sendMessage(from, buff, image, {quoted: ftok, caption: hasilresep})
					break 
					case 'githubstalk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, buff, image, {quoted: ftok, caption: gstalk})
					break 
					case 'infogempa':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, buff, image, {quoted: ftok, caption: gempa})
					break 
				case 'dadu':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
			random = Math.floor(Math.random() * 6) + 1
		damdu = fs.readFileSync(`./sticker/${random}.webp`)
			yamai.sendMessage(from, damdu, sticker, {quoted: mek})
			break
				case 'robot':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await yamai.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
yamai.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediaz = await yamai.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					yamai.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'balik':
	...
	mediau = await yamai.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediao = await yamai.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
...
case 'sider':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
if (!isGroup) return reply(mess.only.group)
infom = await yamai.messageInfo(from, mek.message.extendedTextMessage.contextInfo.stanzaId)
tagg = []
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  yamai.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  ...
	  yamai.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await yamai.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  yamai.sendMessage(from, anu, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await yamai.downloadAndSaveMediaMessage(encmediat)
				anu = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  yamai.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await yamai.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  yamai.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'halloween':
...
case 'matrix':
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				case 'del':
				case 'delete':
					yamai.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
				case 'colong':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
		if (!isQuotedSticker) return reply(`Reply sticker dengan caption *${prefix}colong*`)
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await yamai.downloadAndSaveMediaMessage(encmediia, `./sticker/${sender}`)
		    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
		    if (error) return reply('error')
		    yamai.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                ...
                await sleep(4000)
                yamai.modifyChat(from, ChatModification.delete)
                break
				case 'clearall':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					anu = await yamai.chats.all()
					yamai.setMaxListeners(10)
					for (let _ of anu) {
						yamai.clearMessage(_.jid)
					}
...
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await yamai.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						yamai.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					hah = fs.readFileSync(median)
						yamai.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
			...
			if (!isBotGroupAdmins) return sticNotAdmin(from)
					if (args[0] === 'on') {
						...
						yamai.sendMessage(from, `ALLERT!!! Group ini sudah di pasang anti link\nJika Kamu Melanggar Maka Akan Saya Tendang`, text)
					} else if (args[0] === 'off') {
...
await yamai.sendMessage(from, btnasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
					case 'antibug':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
await yamai.sendMessage(from, btasu, MessageType.buttonsMessage, {quoted: ftrol})
					}
          break
				case 'tinyurl':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
...
case 'tts':
        ...
        if (args.length < 2) return yamai.sendMessage(from, 'Textnya mana gan?', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                yamai.sendMessage(from, buff, audio, {quoted:mek,ptt:true})
                fs.unlinkSync(rano)
            })
        })
...
break 
		    ...
		    yamai.groupDemoteAdmin(from, [demote])
		    ...
		    yamai.groupMakeAdmin(from, [promote])
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					linkgc = await yamai.groupInviteCode(from)
					yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink grup ${groupName}`
					yamai.sendMessage(from, yeh, text, { quoted: mek })
					break
					case 'resetlinkgc':
         case 'resetlinkgroup':
         case 'revoke':
         ...
         yamai.query({json, expect200: true})
...
await yamai.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: ftrol})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return sticAdmin(from)
				...
				for (let i = 0; i < argzi[1]; i++){
					yamai.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				case 'demoteall':
		if (!isOwner && !mek.key.fromMe) return sticOwner(from)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                yamai.groupDemoteAdmin(from, members_id)
                break
                case 'promoteall':
		if (!isOwner && !mek.key.fromMe) return sticOwner(from)
		if (!isGroup) return reply(mess.only.group)
		...
		for (let mem of groupMembers) {
	   	members_id.push(mem.jid)
                ...
                yamai.groupMakeAdmin(from, members_id)
                break
				case 'setnamegc':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return sticAdmin(from)
              if (!isBotGroupAdmins) return sticNotAdmin(from)
					yamai.groupUpdateSubject(from, `${body.slice(11)}`)
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (!isGroupAdmins) return sticAdmin(from)
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
					yamai.groupUpdateDescription(from, `${body.slice(10)}`)
				...
				if (!isQuotedImage) return reply('Reply imagenya!')
					...
					mediau = await yamai.downloadAndSaveMediaMessage(enmediau)
					await yamai.updateProfilePicture(botNumber, mediau)
					reply('Sukses')
					break
				case 'leave':
				if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await yamai.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							yamai.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await yamai.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							yamai.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await yamai.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							yamai.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
await yamai.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftrol})
						}
...
if (!arg) return reply(`Penggunaan ${prefix}spamsw teks|jumlah`)
				...
				if (isNaN(argzi[1])) return reply(`harus berupa angka`)
				for (let i = 0; i < argzi[1]; i++){
					yamai.sendMessage('status@broadcast', argzi[0], MessageType.text)
                    }
                    break	
				case 'upswteks':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(10)
                    yamai.sendMessage('status@broadcast', teks, MessageType.text)
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    yamai.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`${NamaBot}`}, MessageType.location)
...
if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						...
						buff = await yamai.downloadMediaMessage(encmedia)
						yamai.sendMessage('status@broadcast', buff, sticker)
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await yamai.downloadMediaMessage(encmedia)
						yamai.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await yamai.downloadMediaMessage(encmedia)
						yamai.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					var mediap = await yamai.downloadAndSaveMediaMessage(enmediap)
                    const buffer3 = fs.readFileSync(mediap)
                    yamai.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					media = await yamai.downloadAndSaveMediaMessage(enmedia)
                    const buffer6 = fs.readFileSync(media)
                    yamai.sendMessage('status@broadcast', buffer6, MessageType.video, {mimetype : 'video/gif', caption: `${konti}`})
                    ...
                    yamai.sendMessage('status@broadcast', buffer, MessageType.image, {quoted: mek, caption: `${teksyy}`})
                    reply(`Sukses upload image:\n${teksyy}`)
                        break
					case 'shutdown':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
				return yamai.sendMessage(from, JSON.stringify(eval(process.exit())))
				reply('Okey')
				break
				case 'tomp4':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            case 'tomp3':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					mediad = await yamai.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						yamai.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', duration: 359996400, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
				case 'suit':
...
break
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
					anu = from
			   metadete = await yamai.groupMetadata(anu)
				yamai.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
					case 'getbio':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await yamai.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
...
const sname = yamai.contacts[ambl] != undefined ? yamai.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : yamai.contacts[ambl].notify || yamai.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
            ...
            pictt = await yamai.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            yamai.sendMessage(from, pict, image, {quoted: mek})
            break
				case 'chat':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
            ...
            var org = pc.split("|")[1];
            yamai.sendMessage(nomor+'@s.whatsapp.net', org, MessageType.text)   
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, atetepe, sticker, { quoted: mek })
					break
                case 'ttp':  
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                    if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp ${NamaBot}`)
                    anu1 = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${c}`)
                    yamai.sendMessage(from, anu1, image, {quoted: mek, caption : `${prefix}sticker`})
                    break
				case 'semoji':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
    	   ...
    	   break
            ...
            var nomqm = `${body.slice(5)}@s.whatsapp.net`
					tagq = `@${nomqm.split('@')[0]}` 
					yamai.sendMessage(from, tagq, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
			break
			case 'tagme':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
                  var nomqm = mek.participant
				    tagu = `@${nomqm.split('@s.whatsapp.net')[0]}`
					yamai.sendMessage(from, tagu, text, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true, mentionedJid: [nomqm]}})
					break
				case 'join':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
break
            ...
            file = await yamai.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            var group = await yamai.groupMetadata(from)
            ...
            yamai.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            yamai.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            yamai.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            yamai.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            yamai.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await yamai.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
...
case 'stats':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				var groups = yamai.chats.array.filter(v => v.jid.endsWith('g.us'))
				var privat = yamai.chats.array.filter(v => v.jid.endsWith('s.whatsapp.net'))
				var ram2 = `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB`
					...
					const { wa_version, mcc, mnc, os_version, device_manufacturer, device_model } = yamai.user.phone
					stamtus = `⍟ ────────────────── ⍟
...
MNC : ${mnc}
MCC : ${mcc}
Device Model: ${yamai.user.phone.device_model}
Device Manufactur : ${device_manufacturer}
Wa Version: ${yamai.user.phone.wa_version}
Os Version: ${yamai.user.phone.os_version}
...
break
				...
				yamai.updatePresence(from, Presence.composing)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await yamai.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							yamai.sendMessage(_.jid, buff, audio, { quoted: ftrol })
						}
						} else if (isMedia && !mek.message.videoMessage || isQuotedSticker) {
						const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await yamai.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							yamai.sendMessage(_.jid, buff, sticker, { quoted: ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
							}
							} else{
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				var bot = gh.split("|")[2];
			    yamai.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${parti}@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { conversation: `${targetq}` }}})
					break
            case 'hidetag':
            if (!isOwner && !mek.key.fromMe) return sticOwner(from)
            ht = body.slice(9)
            ...
            if (!isGroup) return reply(mess.only.group)
				...
				yamai.sendMessage(from, cs[2], MessageType.text, target)
				break
              case 'hacked':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
              if (!isGroup) return reply(mess.only.group)
              ...
              reply('Otw Hack')
                tessgc = await getBuffer(`https://i.ibb.co/m4Qx3JG/20210319-204838.jpg`)
                   yamai.updateProfilePicture (from, tessgc)
                   await sleep(1000)
                yamai.groupUpdateSubject(from, `HACKED BY ${body.slice(8)}`)
                await sleep(1000)
                yamai.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
                yamai.sendMessage(from, 'Succes Hacked', text, {quoted: mek})
					break
					case 'hack':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
              if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              yamai.groupUpdateSubject(from, `HACKED BY DENZ`)
                yamai.groupUpdateDescription(from, `_${me.jid}_`)
             yamai.updateProfilePicture(from, fs.readFileSync('./media/image/me.jpg'))
                yamai.sendMessage(from, 'Succes!', text, {quoted: mek})
                await sleep(3000)
                yamai.groupLeave(from)
					break
					case 'bugpc2':
...
for (let i = 0; i < args[0]; i++) {
yamai.sendMessage(from, `${NamaBot}`, MessageType.extendedText,{
 quoted: {
key: {
...
break
					...
					if (args.length < 1) return reply('Jumlahnya?')
				 for (let i = 0; i < args[0]; i++) {
await yamai.toggleDisappearingMessages(from, 0)
}
...
yamai.toggleDisappearingMessages(jid, WA_DEFAULT_EPHEMERAL)
}}	
...
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `${NamaBot}`
		        var grousp = await yamai.groupMetadata(from)
		        ...
		        for (let i = 0; i < args[0]; i++) {
		        yamai.sendMessage(from, options, text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
		          ...
		          for(let i=0;i<10;i++){
		        yamai.sendMessage(from, 'Berhasil Mengirim Bug Troli2', text, {quoted:{ 
	    key: {
	    fromMe: false, 
	    participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "62882248593508@s.whatsapp.net" } : {}) 
	    },
...
case 'bugcombine':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
                 yamai.toggleDisappearingMessages(from, 'HACKED')
yamai.sendMessage(from, `${NamaBot}`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
yamai.updatePresence(from, Presence.composing)
		       yamai.sendMessage(from, {degreesLatitude: 34.0184,
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
yamai.updatePresence(from, Presence.composing)
yamai.sendMessage(from, 'yaudah oke',text, {
 quoted: {
  key: {
...
*🔖 Link:* ${items[i].url}\n\n`
                ...
                await yamai.sendMessage(from, thumb, image, {quoted: ftrol, caption: `${hehe}───────────────\n
*🔖 DOWNLOAD*
...
🔖 ${prefix}ytmp4 [link yt] = Video`, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`SEARCH: ${args[0]}`,body:"🔖YOUTUBE SEARCH",mediaType:"2",thumbnail:pporigi,mediaUrl:`https://youtu.be/JN_Gw2GzuqQ`}}})
            ...
            break
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
							const media = await yamai.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
							await ffmpeg(`${media}`)
									.input(media)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											...
											yamai.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
											fs.unlinkSync(media)	
			...
			case 'ping':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				...
				if (!isOwner) return sticOwner(from)
					return yamai.sendMessage(from, JSON.stringify(eval(body.slice(8))), text, {quoted: mek})
					if (err) return yamai.sendMessage(from, `root @dcode-denpa:~ ${err}`, text, { quoted: mek })
                 break
                 case 'toimg':
				case 'tomedia':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					if (mek.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated === true){
						const encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
						const media = await yamai.downloadAndSaveMediaMessage(encmedia)
              ...
              sticWait(from)
				...
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					yamai.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					case 'addvn':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					yamai.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listvn*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
				case 'addimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					...
					yamai.sendMessage(from, `Sukses, silahkan cek dengan *${prefix}listimage*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					try {
              ...
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
					...
					yamai.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
				case 'sticktag':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				if (!isGroup) return reply(mess.only.group)
				anu  = body.slice(10)
				wanu = anu.split('|')
				var group = await yamai.groupMetadata(wanu[0])
				var member = group['participants']
				var mem = []
				member.map( async adm => {
				mem.push(adm.id.replace('c.us', 's.whatsapp.net'))
				...
				yamai.sendMessage(`${wanu[0]}`, result, sticker, { contextInfo: { "mentionedJid": mem }})
				break
			case 'runtime':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: ftrol})
				runtime = process.uptime()
...
await yamai.sendMessage(from, btngrass, MessageType.buttonsMessage, {quoted: ftrol})
					}
					break
			case 'addrespon':
			if (!isOwner && !mek.key.fromMe) return sticOwner(from)
...
Giliran = @${tty.player1.split('@')[0]}`
  yamai.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
  }
...
naa = ky_ttt.filter(toek => !toek.id.includes(from)) 
ky_ttt = naa
yamai.sendMessage(from, `Yahh @${tty.player2.split('@')[0]} Menolak:(`,text,{quoted:mek,contextInfo:{mentionedJid:[tty.player2]}})
}
}
...
${ttt}`
yamai.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player1]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
${ttt}`
yamai.sendMessage(from, ucapan1, text, {quoted:mek, contextInfo:{mentionedJid: [tty.player2]}})
naa = ky_ttt.filter(hhg => !hhg.id.includes(from))
return ky_ttt = naa
}
...
Giliran = @${tty.player1.split('@')[0]}`
 yamai.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     yamai.sendMessage(`${recode.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ${NamaBot}",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./Images/Yamai.jpg'),sourceUrl:"https://ERROR SISTEM"}}})
	}
    }
    }
    }
